  ...
  if (!authChecked) {
    return (
      <div style={{ minHeight: '100vh', background: '#0a0a0a', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <span style={{ fontSize: 11, letterSpacing: '0.2em', color: '#666' }}>...</span>
      </div>
    )
  }
    ...
    <div style={{ minHeight: '100vh', background: '#0a0a0a' }}>
      <header style={{ borderBottom: '1px solid #1a1a1a', padding: '24px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <div style={{ fontSize: 10, letterSpacing: '0.25em', color: '#888', marginTop: 2 }}>BACK OFFICE — {products.length} PIÈCES</div>
        </div>
      ...
      <main style={{ padding: '40px' }}>
        {loading ? (
          <div style={{ color: '#888', letterSpacing: '0.2em', fontSize: 11 }}>CHARGEMENT...</div>
        ) : (
            ...
            {products.map((product) => (
              <div key={product.id} style={{ display: 'flex', alignItems: 'center', gap: 24, padding: '16px 0', borderBottom: '1px solid #1a1a1a' }}>
                <div style={{ width: 48, height: 64, background: '#111', flexShrink: 0, overflow: 'hidden', position: 'relative' }}>
                  {product.images?.[0]
                    ? <img src={optimizeImage(product.images[0])} alt={product.name ?? ''} loading="lazy" style={{ width: '100%', height: '100%', objectFit: 'cover', position: 'absolute', top: 0, left: 0 }} />
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 10, letterSpacing: '0.1em', color: '#888', marginTop: 4 }}>
                ...
                <div style={{ display: 'flex', gap: 20, alignItems: 'center', flexShrink: 0 }}>
                  <button onClick={() => toggleAvailability(product.id, product.is_available)}
                    style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: 10, letterSpacing: '0.15em', color: product.is_available ? '#5a8a5a' : '#777' }}>
                    {product.is_available ? 'DISPONIBLE' : 'LOUÉ'}
                  </button>
                  <Link href={`/admin/modifier/${product.id}`} style={{ fontSize: 10, letterSpacing: '0.15em', color: '#888', textDecoration: 'none' }}>MODIFIER</Link>
                  <button onClick={() => deleteProduct(product.id)}
                    style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: 10, letterSpacing: '0.15em', color: '#664444' }}>SUPPRIMER</button>
                </div>
              </div>
            ))}
            {products.length === 0 && (
              <div style={{ padding: '60px 0', color: '#888', letterSpacing: '0.2em', fontSize: 11, textAlign: 'center' }}>
                AUCUNE PIÈCE —{' '}
                <Link href="/admin/nouveau" style={{ color: '#888', textDecoration: 'none' }}>AJOUTER</Link>
              </div>
            )}
          </div>
        )}
